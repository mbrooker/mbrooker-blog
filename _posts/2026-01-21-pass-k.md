...
1. <a name="foot1"></a> It's mildly interesting how none of the image generation models reliably generate images of legal dice.
...
for (let i = 0; i <= 10; i++) {
  const y = canvas.height - paddingBottom - (i / 10) * graphHeight;
  ctx.beginPath();
  ctx.moveTo(paddingLeft, y);
  ctx.lineTo(canvas.width - paddingRight, y);
  ctx.stroke();
  ctx.fillText((i * 10) + '%', paddingLeft, y + 4);
}

// Draw x-axis labels
for (let k = 0; k <= maxK; k += 5) {
  const x = paddingLeft + (k / maxK) * graphWidth;
  ctx.fillText(k, x - 5, canvas.height - paddingBottom + 20);
}
...
for (let k = 1; k <= maxK; k++) {
  ...
  const x = paddingLeft + (k / maxK) * graphWidth;
  const y = canvas.height - paddingBottom - passK * graphHeight;
