...
Here are the results, as a cumulative latency distribution, comparing FCFS with Nudge:
...
Which is almost ubiquitous in practice. It's very hard indeed to run a stable distributed system where job sizes are allowed to have unbounded cost. Whether our tails are *bounded enough* for Nudge to behave well is a good question, which we can also explore with simulation.
