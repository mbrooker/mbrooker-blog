...
If you're not familiar with Rust, you need to know about [Result](https://doc.rust-lang.org/std/result/enum.Result.html#method.unwrap), a kind of struct that can contain either a successful result, or an error. `unwrap` says basically "return the successful results if there is one, otherwise crash the program"<sup>[1](#foot1)</sup>. You can think of it like an `assert`.
