...
More technically, what's going on here is the *inspection paradox*. Alex and Alice don't experience your latency distribution $f(t)$, they experience a t-weighted version of it. If you have a MTTR or mean request time of $\mathbb{E}[X]$, Alex and Alice experience a mean recovery time $\mathbb{E}_a[X]$ where $\mathbb{E}_a[X] = \frac{\mathbb{E}[X^2]}{2 \mathbb{E}[X]} = \mathbb{E}[X] + \frac{\mathrm{Var}(X)}{2 \mathbb{E}[X]}$.
...
<p>
  Median: <input type="number" id="medianInput" value="200" min="1" step="1" style="width: 5em;"> ms
  &nbsp;&nbsp;
  p99: <input type="number" id="p99Input" value="1000" min="2" step="1" style="width: 6em;"> ms
</p>
...
(function () {
  ...
  function draw() {
    ...
    // Plot relative density (tallest curve peaks at 1) so the y-axis reads in
    // round numbers regardless of the absolute density scale.
    for (let i = 0; i <= N; i++) { servicePts[i] /= peak; customerPts[i] /= peak; }
    const yMax = 1.08;
    ...
    const yStep = niceStep(1, 5);
    for (let v = 0; v <= 1 + 1e-9; v += yStep) {
      const y = yOf(v);
      ctx.beginPath();
      ctx.moveTo(padLeft, y);
      ctx.lineTo(padLeft + plotW, y);
      ctx.stroke();
      ctx.fillText(v.toFixed(1), padLeft - 10, y);
    }
